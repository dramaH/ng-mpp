...
{
    /// <summary>
    ///   The collection of extended attribute (custom field)
    ///   definitions associated with the project.
    ///   Each of the individual entries in the extended attributes
    ///   definition collection.  There are no limits to the number of
    ///   children that may appear, but Project only understands
    ///   Flag1-Flag10, etc.  There must be at least one
    ///   ExtendedAttribute in each ExtendedAttributes collection.
    /// </summary>
    [Table("ExtendedAttributes", Schema = "mpp")]
    public class ExtendedAttribute : MppAggregateRoot<Guid>
    {
        public ExtendedAttribute(): base(Guid.NewGuid()) { }
        public ExtendedAttribute(Guid id): base(id) { }

        // This corresponds to the PID of the custom field. maxLength:
        // N/A
