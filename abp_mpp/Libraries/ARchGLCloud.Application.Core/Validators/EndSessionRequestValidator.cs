...
namespace ARchGLCloud.Application.Core.Validators
{
    public class EndSessionRequestValidator : IEndSessionRequestValidator
    {
        ...
        public EndSessionRequestValidator(IdentityServerOptions options, IHttpContextAccessor context,
                                          ITokenValidator tokenValidator, IRedirectUriValidator uriValidator,
                                          IUserSession userSession, IClientStore clientStore,
                                          IMessageStore<EndSession> endSessionMessageStore, ILoggerFactory loggerFactory)
        {
            Context = context;
            Options = options;
            ...
            Logger = loggerFactory.CreateLogger<EndSessionRequestValidator>();
        }
        ...
        protected virtual EndSessionValidationResult Invalid(string message, ValidatedEndSessionRequest request = null)
        {
            ...
            if (request != null)
            {
                foreach (var cid in request.ClientIds)
                {
                    Logger.LogError(message + Environment.NewLine + $"{cid}");
                }
            }
            else
            {
        ...
        protected virtual void LogSuccess(ValidatedEndSessionRequest request)
        {
            foreach (var cid in request.ClientIds)
            {
                Logger.LogDebug("End session request validation success" + Environment.NewLine + $"{cid}");
            }
        }
        ...
        public async Task<EndSessionCallbackValidationResult> ValidateCallbackAsync(NameValueCollection parameters)
        {
            ...
            var endSessionId = parameters["endSessionId"];
        ...
        protected virtual async Task<(IEnumerable<string> frontChannel, IEnumerable<BackChannelLogoutModel> backChannel)> GetClientEndSessionUrlsAsync(EndSession endSession)
        {
            ...
            {
                Logger.LogWarning("GetClientEndSessionUrlsAsync clientIds : {clientId}");
